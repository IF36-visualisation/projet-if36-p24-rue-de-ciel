...
**Type de ROI le plus faible :** Le genre western présente le retour sur
investissement le plus faible. Cela peut indiquer les coûts de
production élevés et l'attrait limité de ces types de films pour le
public sur les marchés contemporains.
...
data <- read_csv("data/movie_statistic_dataset.csv")
...
data <- read.csv("data/genre_percentage_comparison.csv")
...
-   **L'action** est nettement plus populaire dans le cinéma indien
    qu'aux États-Unis, ce qui peut refléter les préférences différentes
    des deux marchés pour le genre action.

-   Certains genres tels que **la science-fiction (Sci-fi) et la
    musique** sont relativement populaires aux États-Unis et moins
    importants en Inde, ce qui peut être lié aux préférences culturelles
    ou à la structure de l'industrie.

### Conclusion

Le marché cinématographique américain est relativement ouvert à divers
genres, tandis que le marché indien se concentre davantage sur certains
genres (par exemple, les drames et les comédies). Parallèlement, le
graphique révèle que certains genres (par exemple les films d'action)
ont un attrait particulier en Inde, ce qui peut être lié aux préférences
spécifiques du public du pays ou aux stratégies promotionnelles de
l'industrie cinématographique et télévisuelle nationale.
